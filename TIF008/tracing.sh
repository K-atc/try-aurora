...
cp $target_libtiff/tiff_read_rgba_fuzzer_trace $EVAL_DIR/
# $EVAL_DIR/tiff_read_rgba_fuzzer_trace $EVAL_DIR/seed/crash-000117 # => [!] Aborted by canary TIF008
llvm-objdump-16 -d -S $EVAL_DIR/tiff_read_rgba_fuzzer_trace > $EVAL_DIR/tiff_read_rgba_fuzzer_trace.objdump

rm -rf $EVAL_DIR/traces
mkdir -p $EVAL_DIR/traces
### requires at least python 3.6
cd $AURORA_GIT_DIR/tracing/scripts
python3 tracing.py "$EVAL_DIR/tiff_read_rgba_fuzzer_trace @@" $EVAL_DIR/inputs $EVAL_DIR/traces
